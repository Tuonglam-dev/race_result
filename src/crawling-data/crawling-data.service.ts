...
import {
  ...
  RaceResultSchema,
  RaceResultSchema,
} from 'src/schema/race_result.schema';
...
class NewCrawlingService {
  public async NewBrowser(years: number): Promise<Page> {
    ...
    const gotoUrl =
      process.env.CRAWLING_URL + years + process.env.CRAWLING_FIND;
...
export class CrawlingDataService {
  ...
  private async crawlingFieldName(page: Page): Promise<any> {
    const taleFieldSelector = await page.waitForSelector(
      '.resultsarchive-table > thead > tr',
    );
    const tableField = await taleFieldSelector?.evaluate(
      (el) => el.textContent,
    );
  ...
  public async crawlingFieldData(page: Page): Promise<crawlingDataOutPutDTO[]> {
    ...
    for (let i = 0; i < tableDataList.length; i += 8) {
  ...
  public async CrawlingDataToDB(): Promise<void> {
    ...
    for (let i: number = start_year; i <= currentYear; i++) {
      const pageYears = await this._page.NewBrowser(i);
